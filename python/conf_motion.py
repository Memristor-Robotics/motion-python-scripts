...
def conf_set(c,v):
	if c in config_ints:
		send(b'c' + chr(len(config_bytes) + config_ints.index(c)) + pack32(int(v) << 4))
	elif c in config_bytes:
		send(b'c' + chr(config_bytes.index(c)) + pack32(int(v) << 4))
	elif c in config_floats:
		send(b'c' + chr(len(config_bytes) + len(config_ints) + config_floats.index(c)) + float_to_uint32(float(v), 4))    
	else:
		return False
	return True
